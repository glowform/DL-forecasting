...
for (i in seq(n_train, n_records, n_test)){
  ...
  opt = optimizer_adam(lr = 0.0015) #dla dense wyzsza
  ...
  model %>%
    ...
    layer_dense(128, input_shape = input_shape) %>%
...
write_feather(as.data.frame(rets_pr), here("EXPORTS/rets_bvp_mse_dnn.feather"))
